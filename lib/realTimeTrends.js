...
if (!YT_API_KEY) {
    throw new Error('YouTube API key not configured. Please set the YOUTUBE_API_KEY environment variable.');
}
...
async function getGoogleTrendsData(topic) {
    try {
        ...
        // Google Trends API sometimes returns HTML (302/CAPTCHA) instead of JSON.
        // Detect that and bail out gracefully.
        let interestOverTime = null;
        try {
            const parsed = JSON.parse(interestOverTimeRaw);
            interestOverTime = parsed;
        } catch {
            console.warn('Google Trends interestOverTime returned non-JSON (rate-limited?), skipping.');
        }

        // Real-time trends endpoint is unreliable — skip it to avoid crashes.
...
async function getRelatedTopicsAndQueries(topic) {
    try {
        ...
        try { relatedTopics = JSON.parse(relatedTopicsRaw); } catch {
            console.warn('relatedTopics returned non-JSON (rate-limited?), skipping.');
        }
        try { relatedQueries = JSON.parse(relatedQueriesRaw); } catch {
            console.warn('relatedQueries returned non-JSON (rate-limited?), skipping.');
        }

        if (!relatedTopics && !relatedQueries) return null;
        return { relatedTopics, relatedQueries };
    } catch (error) {
        console.error('Error fetching related topics and queries:', error.message);
        return null;
    }
}
...
function analyzeRelatedData(relatedData) {
    ...
    const topicsList = relatedData.relatedTopics?.default?.rankedList?.[0]?.rankedKeyword;
    if (Array.isArray(topicsList)) {
        result.topics = topicsList.map(topic => ({
    ...
    const queriesList = relatedData.relatedQueries?.default?.rankedList?.[0]?.rankedKeyword;
    if (Array.isArray(queriesList)) {
        result.keywords = queriesList.map(query => ({
            term: query.query,
            score: query.value
        }));
    }
