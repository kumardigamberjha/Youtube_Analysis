...
export async function getYouTubeTrendingTopics(timeRange = '24h', searchTopic = '') {
    const cacheKey = searchTopic
        ? `youtube_trending_topics_${timeRange}_${searchTopic}`
        : `youtube_trending_topics_${timeRange}`;
    ...
    try {
        ...
        if (searchTopic) {
            // Get trending videos specifically for the search topic
            const response = await fetch(
                `https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&q=${encodeURIComponent(searchTopic)}&order=viewCount&publishedAfter=${publishedAfter}&maxResults=50&key=${YT_API_KEY}`
            );
            const data = await response.json();
            const items = data.items || [];
            
            // To provide better scoring, fetch statistics
            if (items.length > 0) {
                ...
                if (videoIds.length > 0) {
                    ...
                    items.forEach(item => {
                        if (item.id && item.id.videoId && statsMap[item.id.videoId]) {
...
function analyzeVideoTopics(videos) {
    ...
    videos.forEach(video => {
        ...
        // Search-result items only carry `snippet` (no `statistics`), so default it.
